...
#Region ### START Koda GUI section ### Form=
	$form1 = GUICreate("RAWBLOCKS", 324, 146, -1, -1, BitOR($ws_minimizebox, $ws_group))
	$starterlabel = GUICtrlCreateLabel("RAWBLOCKS", 56, 24, 215, 30)
	GUICtrlSetFont(-1, 24, 800, 0, "Roblox_Font")
	GUICtrlSetColor(-1, 16711680)
	$connecting = GUICtrlCreateLabel("Connecting to Website", 64, 72, 198, 30)
	GUICtrlSetFont(-1, 14, 400, 0, "Comic Sans MS")
	GUISetState(@SW_SHOW)
#EndRegion ### END Koda GUI section ###
InetGet("http://www.rawblox.tk/version/version.txt", "version.txt", 1)
$file = FileOpen(@ScriptDir & "\" & "version.txt", 0)
Local $line = FileReadLine($file)
FileClose($file)
FileDelete(@ScriptDir & "\" & "version.txt")
If $line = "1.00" Then
Else
	MsgBox(4096, "RAWBLOCKS", "Rawblocks has either shut down or the version you are using is out of date.", 10)
	Exit
EndIf
...
#Region ### START Koda GUI section ### Form=c:\users\dkoby\desktop\archived clients\2014l\robloxplayerbeta\actualjoinform.kxf
	$mainform = GUICreate("RAWBLOCKS OMG", 294, 341, -1, -1)
	$label1 = GUICtrlCreateLabel("RAWBLOCKS OMG", 16, 16, 257, 26)
	GUICtrlSetFont(-1, 20, 800, 0, "Roblox_Font")
	GUICtrlSetColor(-1, 16711680)
	$ipinput = GUICtrlCreateInput("localhost", 16, 56, 121, 23)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$portinput = GUICtrlCreateInput("53640", 160, 56, 121, 23)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$joinbutton = GUICtrlCreateButton("Join Server", 16, 152, 267, 49)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$idinput = GUICtrlCreateInput("User Id", 16, 88, 121, 23)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$username = GUICtrlCreateInput("Username", 160, 88, 121, 23)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$hostbutton = GUICtrlCreateButton("Host Server", 16, 208, 267, 49)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$label2 = GUICtrlCreateLabel("Every once in while, Be sure to clean out your content folder.", 0, 328, 315, 19)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$openbtn = GUICtrlCreateButton("Open A Placefile", 16, 120, 267, 25)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	$solobtn = GUICtrlCreateButton("Play Solo (Corescripts dont load E)", 16, 264, 267, 49)
	GUICtrlSetFont(-1, 8, 400, 0, "Comic Sans MS")
	GUISetState(@SW_SHOW)
#EndRegion ### END Koda GUI section ###
While 1
	...
	Switch $nmsg
		Case $gui_event_close
			Exit
		Case $openbtn
			$rbxlopen = FileOpenDialog("Select a file", @ScriptDir & "\", "Roblox Location File (*.rbxl;)")
			If @error Then
				MsgBox(4096, "", "No File(s) chosen or something went wrong")
				Exit
			EndIf
			If FileExists(@ScriptDir & "\content\temp.rbxl") Then
				FileMove(@ScriptDir & "\content\temp.rbxl", @ScriptDir & "\content\temp" & Random(1, 694201337) & ".rbxl")
				FileCopy($rbxlopen, @ScriptDir & "\content\temp.rbxl")
			Else
				FileCopy($rbxlopen, @ScriptDir & "\content\temp.rbxl")
			EndIf
		Case $joinbutton
			ShellExecute(@ScriptDir & "\RawblocksPlayerTest1.exe", '-a "http://www.rawblox.tk/Login/Negotiate.ashx" -j "http://www.rawblox.tk/Game/join2014.php?port=' & GUICtrlRead($portinput) & "&app=http://www.rawblox.tk/charapp/Custom.php?shirt=502&pants=508&hat=&hair=513&body=&face=305&ip=" & GUICtrlRead($ipinput) & "&username=" & GUICtrlRead($username) & "&id=" & GUICtrlRead($idinput) & '&script=5001" -t "1"')
			WinWaitActive("ROBLOX")
			Exit
		Case $hostbutton
			Sleep(5000)
			ShellExecute(@ScriptDir & "\RawblocksPlayerTest1.exe", '-a "http://www.rawblox.tk/Login/Negotiate.ashx" -j "http://www.rawblox.tk/Game/host2014test.php?port=' & GUICtrlRead($portinput) & '" -t "1"')
			WinWait("ROBLOX")
		Case $solobtn
			ShellExecute(@ScriptDir & "\RawblocksPlayerTest1.exe", '-a "http://www.rawblox.tk/Login/Negotiate.ashx" -j "http://www.rawblox.tk/Game/playsolo.php?script=5001&username=' & GUICtrlRead($username) & '" -t "1"')
	EndSwitch
WEnd
